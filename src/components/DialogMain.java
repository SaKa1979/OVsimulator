...
public class DialogMain extends JPanel {
  ...
  public DialogMain(JFrame frame) {
    ...
    JPanel ovSimulatorPanel = createOVsimulatorPanel();
    JPanel protoPanel = createProtocolPanel();
    JPanel outputPanel = createOutputPanel();
    JMenuBar menuBar = createMenuBar();
    JMenu connectionMenu = createConnectionMenu("Settings");
    JMenuItem serialConnectionMI = createSerialConnectionMI("Serial connection");
    GridBagConstraints constraint = new GridBagConstraints();

    //Set them up
    menuBar.add(connectionMenu);
    connectionMenu.add(serialConnectionMI);

    // Add them to main Dialog
    add(ovSimulatorPanel,constraint);
    add(protoPanel,constraint);
    add(outputPanel,constraint);
...
/*--------------------------------------------------------------------------------------------------*/
  /** Creates OVsimulator panel. */
  private JPanel createOVsimulatorPanel() {
    //create this panel
    JPanel ovSimulatorPanel = new JPanel(new GridLayout(10,2));
    //Create the components.     
    Border simpleBlackLineBorder = BorderFactory.createLineBorder(Color.black);
    TitledBorder ovSimulatorBorder = BorderFactory.createTitledBorder(simpleBlackLineBorder, "OV messages");
    //Set them up
    ovSimulatorPanel.setBorder(ovSimulatorBorder);
    ovSimulatorPanel.setMinimumSize(new Dimension(200,200));

    final int numButtons = 20;
    JButton[] buttons = new JButton[numButtons];

    for (int i = 0; i < numButtons; i++) {
      buttons[i] = new JButton("" + i);
      buttons[i].setSize(new Dimension(50,50));
      ovSimulatorPanel.add(buttons[i]);
    }

    return ovSimulatorPanel;
  }
/*--------------------------------------------------------------------------------------------------*/
  /** Creates protocol panel. */
  private JPanel createProtocolPanel(){
     final int PROTOCOUNT = 3;
    //Create this panel
    JPanel protocolPanel = new JPanel(new GridLayout(1,3));
    //Create the components
    Border simpleBlackLineBorder = BorderFactory.createLineBorder(Color.black);
    TitledBorder protocolBorder = BorderFactory.createTitledBorder(simpleBlackLineBorder, "Protocol");
    JRadioButton[] protoRadioButtons = new JRadioButton[3];
    ...
    for (int i = 0; i < PROTOCOUNT; i++) {
      ...
      protocolPanel.add(protoRadioButtons[i]);
    }
    
    //Set them up
    protocolPanel.setBorder(protocolBorder);
...
/*--------------------------------------------------------------------------------------------------*/
  ...
  private JPanel createOutputPanel() {
    //create this panel
    JPanel outputFoundationPanel = new JPanel(new FlowLayout());    /* used as foundation to place the outputTextArea*/
    // Create the components
    Border simpleBlackLineBorder = BorderFactory.createLineBorder(Color.black);
    TitledBorder OutputBorder = new TitledBorder(simpleBlackLineBorder, "ITC communication");
    JScrollPane scrollpane = new JScrollPane();
    scrollpane.setBounds(0,0,150,150);
    //Set them up
    outputFoundationPanel.add(scrollpane);
    outputFoundationPanel.setBorder(OutputBorder);
    
        outputText = new JTextArea("Reply from ITC.");
        outputFoundationPanel.add(outputText);
        outputText.setEditable(false);
        outputText.setWrapStyleWord(true);
        outputText.setLineWrap(true);

    return outputFoundationPanel;
  }
  /*--------------------------------------------------------------------------------------------------*/
  /** Creates the menuBar. */
  ...
  /*--------------------------------------------------------------------------------------------------*/
  /** Creates the connectionMenu. */
  private JMenu createConnectionMenu(String name){
    return new JMenu(name);
  }
  /*--------------------------------------------------------------------------------------------------*/
  /** Creates the serialConnectionMI. */
  private JMenuItem createSerialConnectionMI(String name){
